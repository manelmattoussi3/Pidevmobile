...
public class PublicationForm  extends BaseForm{
    ...
    public PublicationForm(Resources res) {
        ...
        ev=publicationService.getListPub();
            for(Publication publication : ev) {
                 String url ="http://127.0.0.1:8000/api/publication/image?file="+publication.getFile();
    EncodedImage placeholder = EncodedImage.createFromImage(Image.createImage(this.getWidth() / 2, this.getHeight() / 3, 0xFFFFFFFF), true);
    Image images = URLImage.createToStorage(placeholder, url, url,URLImage.RESIZE_SCALE_TO_FILL);
 
                
            Display.getInstance().callSerially(() -> {}); 
        addButton(images,publication.getTitre(),publication.getDescription(),publication);
 ...
 //--------------------------bch nfsrouh --------------------------------------//
    ...
    public void templateBack(Resources res) {
        Tabs swipe = new Tabs();
        ...
        swipe.hideTabs();
        addTab(swipe,res.getImage("stat.jpg"), "");
 ...
 //--------------------------bch nfsrouh --------------------------------------//
        ...
        RadioButton mesEv = RadioButton.createToggle("Acceuil", barGroup);
        mesEv.setUIID("SelectBar");
        
        RadioButton modif = RadioButton.createToggle("Publications", barGroup);
        ...
        add(LayeredLayout.encloseIn(
                GridLayout.encloseIn(2, mesEv, modif),
    ...
    public void templateFront(Resources res) {
        ...
        swipe.getContentPane().setUIID("Container");
        swipe.hideTabs();
        addTab(swipe,res.getImage("feedback.jpg"), "");
        ButtonGroup bg = new ButtonGroup();
        int size = Display.getInstance().convertToPixels(1);
        Image unselectedWalkthru = Image.createImage(size, size, 0);
        Graphics g = unselectedWalkthru.getGraphics();
